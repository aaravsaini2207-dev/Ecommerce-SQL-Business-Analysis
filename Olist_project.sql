...
#Q5: Revenue trend
select DATE_FORMAT(o.order_purchase_timestamp, '%Y-%m') as order_date,
		ROUND(sum(oi.price),2) as monthly_revenue,
        ROUND(sum(sum(oi.price)) OVER (order by DATE_FORMAT(o.order_purchase_timestamp, '%Y-%m')),2) as running_total
from order_items as oi
join orders as o 
	on oi.order_id = o.order_id
group by order_date
...
with data as (select DATE_FORMAT(o.order_purchase_timestamp, '%Y-%m') as months,
		ROUND(sum(oi.price),2) as revenue, ROUND(sum(oi.price-oi.freight_value),2) as profit
        from order_items as oi
        join orders as o
        on oi.order_id=o.order_id
        group by months),
...
with data as (select p.product_category_name,
		round(sum(oi.price),2) as revenue, round(sum(oi.price- COALESCE(oi.freight_value,0)),2) as profit      #coalesce to treat all null values as 0
        from order_items as oi
        join products as p
        on oi.product_id=p.product_id
        group by product_category_name),
...
#: Segment Customers by Revenue  High Value → revenue > 1000  Medium → 300–1000  Low → < 300
...
select customer_unique_id,total_revenue,
     case   
			...
			when total_revenue between 200 and 1000 then 'Medium_Value'
            else 'Low_Value'
...
SELECT months,revenue,
		ROUND(lag(revenue) OVER(order by months),2) as previous_year_revenue,
...
with data_c as (select c.customer_unique_id,
						count(distinct o.order_id) as total_orders
						from orders as o
                        join customers as c
                        on o.customer_id=c.customer_id
